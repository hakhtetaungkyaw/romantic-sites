...
function withAccentedAmpersands(text: string) {
  return text
    ...
    .map((part, i) =>
      part === "&" ? (
        <span key={i} className="text-[#c9a0a0]">
          &amp;
        </span>
      ) : (
        part
  ...
  return (
    <section className="relative flex min-h-[70vh] flex-col items-center justify-center px-6 text-center">
      <motion.p
        ...
        className="mb-4 text-sm uppercase tracking-[0.3em] text-[#c9a0a0]"
      >
        A love story
      </motion.p>
      ...
      <motion.h1
        ...
        className="font-serif text-5xl font-medium text-[#2c2420] sm:text-6xl md:text-7xl"
      >
      ...
      <motion.h2
        ...
        className="mt-6 max-w-xl font-serif text-xl italic text-[#2c2420]/75 sm:text-2xl"
      >
        {title}
      </motion.h2>
    </section>
