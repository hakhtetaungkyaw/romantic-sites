import GiftUnwrap from "@/components/birthdayShared/interactive/GiftUnwrap";
import { dummyBirthdayData } from "@/lib/dummyData";
import type { BirthdayCustomData } from "@/types/site";

// Temporary, phase-scoped preview — renders only GiftUnwrap in isolation so
// it can be reviewed before the rest of the Birthday V1 "Celebration Room"
// template is built out. Safe to delete once that lands. Same pattern as
// app/preview/birthday-gate/page.tsx and app/preview/balloon-reveal/page.tsx.
export default function GiftUnwrapPreviewPage() {
  const birthday = dummyBirthdayData.customData?.birthday as BirthdayCustomData;

  return (
    <main className="flex min-h-screen items-center justify-center bg-[#ECE9E6] py-16">
      <GiftUnwrap
        giftMessage={birthday.giftMessage}
        giftLayerTwoPhrase={birthday.giftLayerTwoPhrase}
        giftWheelItems={birthday.giftWheelItems}
      />
    </main>
  );
}
